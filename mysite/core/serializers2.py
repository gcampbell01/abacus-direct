...
def json_serialize(obj, **kwargs):
    '''serialize a python object'''
    js = JSONSerializer()
    
    j = js.serialize(obj, **kwargs)
...
class JSONSerializer():
    ...
    def serialize(self, obj, *args, **options):
        ...
        self.fields_call_list = []

        self.stream = options.pop("stream", StringIO())
        self.selectedFields = options.pop(
            "fields", options.get('fields', None))
        self.ignoredFields = options.pop(
            "ignored", options.get('ignored', None))
        self.use_natural_keys = options.pop(
            "use_natural_keys", options.get("use_natural_keys", False))
        self.transcript = options.pop(
    ...
    def handle_object(self, _object):
        ...
        if isinstance(_object, dict):
            self.handle_dictionary(_object)

        elif isinstance(_object, list):
            
            self.handle_list(_object)

        elif isinstance(_object, Model):
            self.handle_model(_object)

        elif isinstance(_object, QuerySet):
            self.handle_queryset(_object)

        elif isinstance(_object, bool):
            self.handle_simple(_object)

        elif isinstance(_object, int) or isinstance(_object, float) or isinstance(_object, long):
            self.handle_simple(_object)

        elif isinstance(_object, unicode):
            self.handle_simple(_object)

        elif isinstance(_object, basestring):
            self.handle_simple(_object)

        elif isinstance(_object, datetime):
            self.handle_datetime(_object)

        elif isinstance(_object, date):
            self.handle_date(_object)

        elif _object == None:
            self.handle_none(_object)
        else:
            # If this a seralize entity
            try:
                self.handle_string(_object)
            except:
                raise UnableToSerializeError(type(_object))
        ...
        if _meta is None:
            ...
            for field in mod._meta.many_to_many:
                if self.selectedFields is None or field.attname in self.selectedFields:
                    if self.ignoredFields is None or self.currentLoc + field.attname not in self.ignoredFields:
                        if field not in self.fields_call_list:
                            self.fields_call_list.append(field)
                            self.handle_m2m_field(mod, field)
                        else:
                            self.handle_m2m_field(mod, field, False)
    ...
    def handle_m2m_field(self, mod, field, recursive=True):
