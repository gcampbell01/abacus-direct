...
'''
>>> from json import loads
>>> from core.serializers_test import main; main()
'''
def main():
	class Ref(object):
		def __init__(self):
	...
	json = json_serialize(test_object, nested=False)

	dict_obj = {}
	dict_s = json_serialize(dict_obj, nested=False)
	list_s = json_serialize([], nested=False)
	empty_s = json_serialize('', nested=False)
	zero_d = json_serialize(0, nested=False)

	assert dict_s == '{"__recursive__": "' + str(id(dict_obj)) + '"}'
	assert list_s == '[]'
	assert empty_s == '""'
	assert zero_d == '0'
